...
test('username correct et pass correct',{tag:['@tc-001', '@smoke',"@regression"]}, async ({ page }) => {
...
test('username incorrect et pass correct',{tag:['@tc-002', '@smoke',"@regression"]}, async ({ page }) => {
  await page.goto('https://www.saucedemo.com/');
  const loginPage:LoginPage= new LoginPage(page);
